...
class Hand:
    def __init__(self, cards: str, bid: int) -> None:
        ...
        # cast letters to A-E so can use string sort
        self.cards = (
    ...
    def get_type(self):
        counter = Counter(self.cards).most_common()
        if counter[0][1] == 5:  # five of a kind
            return 6
        if counter[0][1] == 4:  # four of a kind
            return 5
        if counter[0][1] == 3:  # 3 of one card...
            if counter[1][1] == 2:  # full house
                return 4
            else:  # three of a kind
                return 3
        if counter[0][1] == 2:  # two of one card
